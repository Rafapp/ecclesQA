...
export const FEEDBACK_DRAFTS_STORAGE_KEY = "wandFeedbackDrafts";
const DIAGNOSTIC_LOG_STORAGE_KEY = "wandDiagnosticLog";
...
export type FeedbackDraftInput = Omit<FeedbackDraft, "summary" | "details"> & {
  summary: string;
  details: string;
};
...
export async function getFeedbackDrafts(): Promise<FeedbackDraft[]> {
  const stored = await chrome.storage.local.get(FEEDBACK_DRAFTS_STORAGE_KEY);
  return Array.isArray(stored[FEEDBACK_DRAFTS_STORAGE_KEY])
    ? stored[FEEDBACK_DRAFTS_STORAGE_KEY] as FeedbackDraft[]
    : [];
}

export function formatFeedbackDraft(draft: FeedbackDraft): string {
  const heading = draft.kind === "bug" ? "Wand bug report" : "Wand suggestion";
  const lines = [
    heading,
    `Report ID: ${draft.id}`,
  ...
  if (draft.issueType) {
    lines.push("", `UDOIT issue: ${draft.issueType}`);
  }
  if (draft.sourceTitle) {
    lines.push(`Canvas source: ${draft.sourceTitle}`);
  }
  if (draft.pageUrl) {
    lines.push(`Page: ${draft.pageUrl}`);
  }
  if (draft.diagnostics.length) {
    lines.push("", "Recent diagnostics:", JSON.stringify(draft.diagnostics, null, 2));
  }
